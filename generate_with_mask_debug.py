...
def load_model_from_config(config, ckpt, verbose=False):
    ...
    model = instantiate_from_config(config.model)   ## config.model loads the config file.
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--data_root",
        default="/user/guoxia11/cvl/anomaly_detection/anomaly_detection_dataset/mvtec",
        # required=True,
    )
    parser.add_argument(
        "--sample_name",
    ...
    config = OmegaConf.load("/research/cvl-guoxia11/anomaly_detection_v2/anomalydiffusion/configs/latent-diffusion/txt2img-1p4B-finetune-encoder+embedding.yaml")
    ...
    model.prepare_spatial_encoder(optimze_together=True)    ## GX: is this function important?
    ckpt = torch.load('/research/cvl-guoxia11/anomaly_detection_v2/anomalydiffusion/logs/anomaly-checkpoints/checkpoints/spatial_encoder.pt')
    model.embedding_manager.spatial_encoder_model.load_state_dict(ckpt)
    model.embedding_manager.load('/research/cvl-guoxia11/anomaly_detection_v2/anomalydiffusion/logs/anomaly-checkpoints/checkpoints/embeddings.pt')
    ...
    save_dir = '/research/cvl-guoxia11/anomaly_detection_v2/anomalydiffusion/generated_dataset/%s/%s' % (sample_name, anomaly_name)
